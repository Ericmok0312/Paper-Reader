...
import { X, Save, FileCode, RotateCcw, HardDrive, CheckCircle2 } from 'lucide-react';
...
const SettingsModal: React.FC<SettingsModalProps> = ({ settings, onSave, onClose, onConfigureBackup, backupStatus }) => {
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1 leading-relaxed">
              Select a file on your device. ScholarNote will automatically save your entire library to this file every 60 seconds while the app is open.
            </p>
          </div>
        </div>
          ...
          {backupStatus.active ? (
            <div className="flex flex-col gap-3">
              ...
              <div className="text-[10px] text-slate-400">
                Last saved: {backupStatus.lastBackup ? backupStatus.lastBackup.toLocaleTimeString() : 'Pending...'}
              </div>
              <button 
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 mt-2 italic">
                Note: Browser security may require you to re-confirm permission occasionally.
              </p>
            </div>
          )}
        </div>
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl animate-in zoom-in-95 max-h-[90vh] flex flex-col">
        ...
        <div className="flex border-b border-gray-100 px-6 shrink-0 gap-1">
          <button 
            ...
            className={`py-3 px-3 text-xs font-bold uppercase tracking-wider border-b-2 transition-colors ${activeTab === 'general' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
          >
            General
          </button>
           <button 
            onClick={() => setActiveTab('backup')}
            className={`py-3 px-3 text-xs font-bold uppercase tracking-wider border-b-2 transition-colors ${activeTab === 'backup' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
          >
            Backup
          </button>
          <button 
            onClick={() => setActiveTab('prompts')}
            className={`py-3 px-3 text-xs font-bold uppercase tracking-wider border-b-2 transition-colors ${activeTab === 'prompts' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
          >
            AI Prompts
          </button>
        </div>
